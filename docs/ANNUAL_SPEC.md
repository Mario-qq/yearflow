...
- **留白的三条硬约束**（收尾补丁回填，起因是初版四个方向的留白都不成立）：
  1. **beat 间距必须明显大于卡片内边距**：`PAGE_GAP = 40` vs 卡片 `p-6`(24)。两者取同值时，11 个 beat 读起来是一列均匀格子，「一屏一 beat」当场作废。`exportLong` 的 `STAGE_GAP` 引用同一个常量，长图节奏才与屏幕一致。
  2. **卡片内只许有一条左边缘**：序号与 eyebrow 同一行、标题独占下一行，标题/hero 数字/图三者左边缘对齐。序号若在流里占一列，标题会被推进约 24px，卡里就出现两条左边缘。
  3. **顶部留白挂在 sticky 顶部条自身，不挂在页面列的 `padding-top` 上**。挂在列上时那段留白随滚动划走，条贴顶后又变回零间距。
- `CHART_W = PAGE_W − 页面 padding(24×2) − 卡片 padding(24×2) = 804`。**改卡片内边距必须同步改它**，7 张自绘图都从这里取宽。
- 深浅主题一等公民，两套都过截图门槛。
...
- **宽图横滚，不等比压扁**。`CHART_W`(804) 等比缩到 343px 屏宽时，`--font-11` 的轴标签只剩 4.6px —— 图还在但读不了。所以窄屏下 SVG 保持 `MOBILE_CHART_W`(720)、外层容器横向滚动（与宽表格的通行做法一致）。这条落在 `ChartBox` 里，一处改动覆盖全部 7 张自绘图。
