...
### 八、小窗透明度 + 窗内选事项（2026-08-20 追加）

**透明度**：`PomodoroSettings.pipOpacity`（默认 100，范围 [30, 100]，五处同步——domain 类型、defaults、backup zod schema、constants 上下限、设置页滑块）。渲染进程是权威，主进程只缓存最后收到的值，于是「关小窗→再开」不会闪一下 100% 再变暗。clamp 在主进程再做一遍：`setOpacity(0)` 会让窗口彻底看不见却仍在置顶抢点击，属不可恢复状态。下限 30 是硬约束——再低就看不清倒计时，而看得清是这个窗存在的全部理由。设置项只在 `isDesktop()` 下渲染：web 版的 Document PiP 没有这个能力，摆一个点了没反应的滑块比没有更糟。

**窗内选事项**：需求原话是「不影响美观或者大小」，所以两条都按字面兑现——
- 触发口塞进**本来就存在的** 28px 顶栏：`● 专注 🧩 SAP系统 · SD 模块`，靠 `min-width: 0` + 省略号收口。200px 最窄态实测段点仍在窗内（segs right=168 / 窗宽 200）、事项名 `scrollWidth > clientWidth` 即真被截断。顶栏顺带加了 `gap: 8px`，否则截断后的省略号紧贴段点。
- 选择器是**覆盖整窗的浮层**，只在打开时存在 ⇒ 关掉后小窗几何一模一样（自查里显式断言了打开前后 `innerWidth/innerHeight` 不变）。
- **专注中不可改归属**（顶栏那段退化为静态文本）：中途改归属等于篡改已经发生的记录。空闲与休息中可改，改的是「下一段用哪个」。
- 候选集抽成 `useFocusOptions()` 供两个消费者共享（主面板下拉 + 小窗浮层）。那套分组规则并不显而易见（随缘任务不在 `dayEntries` 里、「最近」刻意不受 `noFocus` 与「今日在办」约束、脏 id 要滤掉），复制一份必然走样。呈现方式两边各自最优——主面板那套「锚点+视口翻转下拉+搜索框+每行悬浮小按钮」在 200px 里根本放不下。`refreshRecent` 用 `useCallback` 固定引用，因为消费者会把它放进 `useEffect` 依赖数组。
...
`tsc -b` + `oxlint` 干净，`vitest` 225 全绿。`desktop:e2e` **31 项全过**（新增 E 段 10 项：按钮存在性、浮层候选数、浮层不改尺寸、选择落 localStorage 且主窗同源、从小窗起的专注带上所选事项、专注中锁死、200px 截断与段点不越界；F 段 2 项：透明度 60%/100% 在原生窗口实际生效）。`desktop:smoke` 打包形态 **18 项全过**。小窗新增 3 张截图人工过目（浮层、带事项运行态、200px 窄态）。

### 十一、首版明确未做

托盘图标、自动更新、代码签名、NSIS/portable 安装包（见 §六、§七）、原生保存对话框（备份导出继续走 anchor 点击）、以及「把多 tab 机制简化掉」的重构。

### 十二、小窗缩到 20% + 贴边收起（2026-08-21）

需求原话：「悬停靠在桌面一边上然后只显示倒计时」「整个窗体只留 20% 的大小」。于是小窗从 260×172 的四条带常驻，改成三态：

| 态 | 尺寸 | 显示 |
|---|---|---|
`free`（常态） | **116×76**（≈原面积的 20%） | 只有 mm:ss + 3px 进度线 |
`free` + hover | 116×76，**尺寸不变** | 窗内浮出：阶段点 / 事项名 / 收起 / 控制行 / 段点 |
`docked`（贴边） | **88×30** | 紧贴屏幕某边的窄药丸，只有 mm:ss + 进度线 |
`docked` + hover | 116×76 | 临时展开（peek），移开鼠标 400ms 后收回 |

**为什么控件是「窗内浮出」而不是「hover 放大窗口」**：变尺寸的方案在屏幕边缘会与 hover 判定互相触发——展开后光标可能落到窗外 → leave → 收回 → enter → 抖个不停。只有 docked ⇄ peek 这一处必须改尺寸，那里靠主进程的状态机 + 「已吸在这条边就什么都别做」兜住。

**⚠️ 浮层是条件挂载，不是常挂着切 opacity。** `-webkit-app-region` 是原生 hit-test，与 `opacity` / `pointer-events` 完全无关：常挂一层带 `no-drag` 按钮的浮层，会在收起态留下一片「拖不动窗、又能误关窗」的死区——正是 §九.3 那个坑的同一张脸。同理 `.pip-native-close` 在未 hover 时显式把自己改回 `drag`。整块窗体现在都是拖动区（这么小的窗，哪都能拖才顺手），可交互元素逐个 `no-drag`。

**几何真相全在主进程**（`electron/main.cts` 的 dock 状态机）。渲染进程只报鼠标进出、只请求 dock/undock，因为往屏内哪个方向展开、怎么 clamp 进 workArea，只有主进程知道。三个坑各花了一轮自查：

1. **`moved` 收不到程序化移动。** Windows 上拖动中连发 `move`、松手补一个 `moved`，而 `setBounds` 只发 `move`。只听 `moved` 的话自查（以及任何程序化摆窗）一条都收不到 ⇒ 两个都听，防抖 120ms 收敛到同一次判定。
2. **「这是我们自己摆的」不能用时间窗判定。** 原先 `ignoreMovedUntil = now + 400ms`，结果紧跟其后的一次真实移动被整个吞掉，之后没有事件补发 → 窗停在边上却不吸附。改成比对**目标矩形**，没有盲区。
3. **外框尺寸不能读出来再写回去。** 无边框窗在 Windows 上带一圈不可见边框，`getBounds` 比请求值大 1–3px；这套逻辑要反复「读当前位置 → 算 → 写回」，用外框每来回一次窗就胖 2px（实测 116→119→121→125）。全部改走 `getContentBounds` / `setContentBounds`，读写同口径、不累积。另：药丸高度定 30 而非 26，因为可拉伸窗的最小高度会被系统夹到 ~30，所以 dock 时先 `setResizable(false)` 再改尺寸。
4. **脱离边缘后必须把窗推离边缘至少 SNAP+2。** 不推的话 undock 把窗摆回原处（仍贴着边）→ 这次 `setBounds` 的 move 撞上吸附判定 → 立刻又收起来，「脱离边缘」从外面看根本没发生过。

**位置与吸附态持久化**：`userData/pip-window.json`（防抖 500ms 写，关窗时 flush）。**刻意不进 Dexie、不进 `lib/backup.ts`**——本机窗口偏好不是用户数据，不该进备份也不该跟着云同步跑到别的机器上。顺带补上了此前一直缺的「关窗再开位置还在」。

**版式收档**（`constants.ts` 一处改完）：主按钮 36→22、次按钮 30→26→18、图标 14→10、段点 5→4、印章 40→28、顶行 28→20、纸屑每筒 26→14。倒计时改 `min(--font-48, max(--font-32, 34vh))`：常态 32px，用户把窗拉大时数字跟着长。段点从顶行挪到控制行右端（116px 的顶行装不下事项名 + 两颗窗控 + 段点）；阶段文案退到状态点的 `title`；到点庆祝屏砍掉顶栏与副行（副行退到主句的 `title`）。事项选择浮层字号与行距同步收档、目标名整条让位给任务名。

**又一个只有看图才发现的 bug**：`.pip-confetti` 只写了 `position:absolute; inset:0`。canvas 是替换元素，会保留 `width/height` 属性那份 intrinsic 尺寸（DPI 缩放后比小窗还大），于是把庆祝屏撑出 149px 的溢出——断言全绿、肉眼也看不出，是新加的「这一屏在 116×76 里不溢出」那条 `scrollHeight - clientHeight` 断言抓到的。补 `width/height: 100%` 后清零。
