...
🔴 **既有 `src/gantt/lib/exportPng.ts` 有同一个 bug**（实测导出 2880×1848、108kB、非背景像素 **0**）。Y3 的改动面禁止碰 `gantt/*`，故只记录不修；修法与上面同构（外壳 fixed，内层静态，`toCanvas(inner)`）。→ **已于 2026-08-14 单独修复，见文末「甘特图导出 PNG 白板修复」。**
...
### 甘特图导出 PNG 白板修复（2026-08-14，Y3 遗留项）

补掉 Y3 记录但按改动面禁令未修的那条 🔴：`src/gantt/lib/exportPng.ts` 交给 `toCanvas` 的就是 `position:fixed; left:-100000px` 的离屏舞台，导出 2880×1848、文件名与体积都对、**非背景像素 0**。

**修法与 `annual/exportLong.ts` 的 `makeStage` 同构**：拆成外层 `host`（`fixed / left:-100000px / z-index:-1` 负责离屏）+ 被截的 `stage`（改 `position:relative`，**只去掉偏移、保留定位上下文**——内层那个 `-scrollLeft/-scrollTop` 的绝对定位层还要靠它当包含块；毒的是负偏移，不是 `position` 本身），`toCanvas(stage)`，`finally` 摘 `host`。改动只有这一个文件、只有舞台那几行，sticky 补偿与视口裁剪逻辑一字未动。

**验收**：新增 `scripts/check-gantt-export.mjs`（Playwright + 系统 Chrome，深浅两主题各点一次顶栏「导出」，把落盘 PNG 送回页面解码数非背景像素）。修后 2880×1648，浅色 **2466191** / 深色 **2497799** 个非背景像素；把修改 `git stash` 掉复跑，同一脚本报 `[light] 不是白板：0 个非背景像素` —— 确认这条断言真的抓得住该 bug，而尺寸/体积断言全程都是绿的。`tsc -b` 干净。

事件总线 `gantt/bus.ts` 是模块内私有 `EventTarget`，页面里够不着，所以脚本只能点顶栏按钮触发，不能 `dispatchEvent`。
