...
/** 卡片内绘图区宽度 = PAGE_W − 页面左右 padding(24×2) − 卡片 padding(24×2) */
export const CHART_W = 804;
/**
 * beat 之间的纵向间距。**明显大于卡片内边距（24）才有节拍感** —— 两者取同一个值时，
 * 11 个 beat 读起来是一列均匀格子，而不是规格 §4.3 要的「一屏一 beat」。
 * exportLong 的 STAGE_GAP 必须与它同值，长图的节奏才与屏幕上一致。
 */
...
/**
 * 移动端自绘图的渲染宽度。**不缩到屏宽**：CHART_W(804) 等比压到 343 时，
