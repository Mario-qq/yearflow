...
/**
 * 窗高不超过这个值就按「收起态」画（只有 mm:ss + 进度线）。
 * 形态不能只信主进程那条 pip:mode 消息 —— 消息丢一次或抢跑一次，窗内那棵树就再也换不
 * 回来（表现为药丸里露出完整态的残片：段点和 ×，一个数字都没有）。窗高是随时能重新
 * 测出来的事实，所以它才是权威。46 卡在 PIP_DOCK_H(30) 与桌面窗最小高(64) 中间，
 * 两侧都留足余量：用户手动把自由态拉到最小，不会被误判成收起。
 */
export const PIP_COMPACT_MAX_H = 46;
